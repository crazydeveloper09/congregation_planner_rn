...
const CartDayDeleteConfirmScreen: React.FC<CartDayDeleteConfirmScreenProps> = ({ navigation, route }) => {
    ...
    return (
        <ScrollView style={styles.container} contentContainerStyle={{ justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Text style={styles.text}>Czy na pewno chcesz usunąć dzień wózka {route.params.cartDay.date}?</Text>
            <View style={{ flexDirection: 'row', gap: 8 }}>
                <View style={{ width: '48%' }}>
                    <ButtonC title="Tak" onPress={() => deleteCartDay(route.params.cartDay._id)} isLoading={state.isLoading} color="#AD371F" />
                </View>
                <View style={{ width: '48%' }}>
                <ButtonC title="Nie" onPress={() => navigation.navigate('Carts Schedule Index')} />
                </View>
