...
> = ({ route }) => {
  ...
  const [participantItems, setParticipantItems] = useState([
    {
      label: meetingAssignmentsTranslate.t("otherCongPreacherChoose"),
      value: "",
    },
  ]);
  ...
  const [typeItems, setTypeItems] = useState(
    route.params.meeting.type === meetingTranslate.t("weekend")
      ? [
          {
            label: meetingAssignmentsTranslate.t("bibleTalk"),
            value: "bibleTalk",
          },
          {
            label: meetingAssignmentsTranslate.t("watchtowerStudy"),
            value: "watchtowerStudy",
          },
        ]
      : [
          {
            label: meetingAssignmentsTranslate.t("treasuresFromGodsWord"),
            value: "treasuresFromGodsWord",
          },
          {
            label: meetingAssignmentsTranslate.t("applyYourselfToMinistry"),
            value: "applyYourselfToMinistry",
          },
          {
            label: meetingAssignmentsTranslate.t("livingAsChristians"),
            value: "livingAsChristians",
          },
        ]
  );
  ...
  const [defaultTopicItems, setDefaultTopicItems] = useState([
    {
      label: meetingAssignmentsTranslate.t("chooseTopicByYourself"),
      value: "",
    },
    {
      label: meetingAssignmentsTranslate.t("spiritualGems"),
      value: meetingAssignmentsTranslate.t("spiritualGems"),
    },
    {
      label: meetingAssignmentsTranslate.t("bibleReading"),
      value: meetingAssignmentsTranslate.t("bibleReading"),
    },
    {
      label: meetingAssignmentsTranslate.t("startConversation"),
      value: meetingAssignmentsTranslate.t("startConversation"),
    },
    {
      label: meetingAssignmentsTranslate.t("followingUp"),
      value: meetingAssignmentsTranslate.t("followingUp"),
    },
    {
      label: meetingAssignmentsTranslate.t("makeDisciples"),
      value: meetingAssignmentsTranslate.t("makeDisciples"),
    },
    {
      label: meetingAssignmentsTranslate.t("explainBeliefs"),
      value: meetingAssignmentsTranslate.t("explainBeliefs"),
    },
    {
      label: meetingAssignmentsTranslate.t("talk"),
      value: meetingAssignmentsTranslate.t("talk"),
    },
    {
      label: meetingAssignmentsTranslate.t("congregationNeeds"),
      value: meetingAssignmentsTranslate.t("congregationNeeds"),
    },
    {
      label: meetingAssignmentsTranslate.t("orgAchievements"),
      value: meetingAssignmentsTranslate.t("orgAchievements"),
    },
    {
      label: meetingAssignmentsTranslate.t("congregationStudy"),
      value: meetingAssignmentsTranslate.t("congregationStudy"),
    },
  ]);
  ...
  const loadPreachers = async (type: string) => {
    ...
    const currentMonthMeetings = state.allMeetings?.filter(
      (meeting) => meeting.month === currentMonth
    );
    const selectParticipantItems = allPreachers!
      .filter(
        (preacher) =>
          preacher.roles.includes("can_have_assignment") &&
          preacher.roles.includes(role)
      )
      .map((preacher) => {
        let alreadyAssigned = 0;
        currentMonthMeetings?.forEach((meeting) => {
          alreadyAssigned += meeting.assignments?.filter(
            (assignment) => assignment.participant?.name === preacher.name
          ).length;
        });
        return {
          label: meetingAssignmentsTranslate.t("assignmentsCounter", {
            name: preacher.name,
            currentMonth,
            alreadyAssigned,
          }),
    ...
    const readerItems = allPreachers!
      .filter((preacher) => preacher.roles.includes("can_be_reader"))
      .map((preacher) => {
        let alreadyRead = 0;
        currentMonthMeetings?.forEach((meeting) => {
          alreadyRead += meeting.assignments?.filter(
            (assignment) => assignment.reader?.name === preacher.name
          ).length;
        });
        return {
          label: meetingAssignmentsTranslate.t("readingCounter", {
            name: preacher.name,
            currentMonth,
            alreadyRead,
          }),
    ...
    const helperItems = allPreachers!
      .filter((preacher) => preacher.roles.includes("can_do_exercise"))
      .map((preacher) => {
  ...
  return (
    <KeyboardAwareScrollView style={styles.container} contentContainerStyle={isDesktop && { width: '50%', marginHorizontal: 'auto'}}>
      ...
      >
        {meetingAssignmentsTranslate.t("seeOtherAssignmentsLabel")}
      </Text>
      <Meeting
        meeting={route.params.meeting}
        filter={mainTranslate.t("all")}
        shouldAutomaticallyExpand={false}
      />
      <Text
        style={[
          styles.meeting,
      ...
      <AudioVideo
        meeting={route.params.meeting}
        audioVideo={route.params.meeting.audioVideo}
        shouldAutomaticallyExpand={false}
      />
      <Text
        style={[
      ...
      >
        {attendantTranslate.t("sectionText")}
      </Text>
      <Attendant
        meeting={route.params.meeting}
        attendant={route.params.meeting.ordinal}
        shouldAutomaticallyExpand={false}
      />
        ...
        defaultTopicValue && (
          <>
            ...
            <DropDownPicker
              ...
              placeholder={meetingAssignmentsTranslate.t(
      ...
      {(typeValue === "watchtowerStudy" ||
        defaultTopicValue ===
          meetingAssignmentsTranslate.t("congregationStudy")) && (
        <>
      ...
      {typeValue === "bibleTalk" && (
        <>
          <Label
            text={meetingAssignmentsTranslate.t(
              "isOtherParticipantSwitchLabel"
            )}
          />
          <Switch
